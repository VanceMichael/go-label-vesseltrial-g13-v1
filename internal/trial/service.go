...
type Repository interface {
	...
	HasOverlappingLeg(context.Context, int64, model.VoyageLeg) (bool, error)
	AddLeg(context.Context, int64, int64, model.VoyageLeg, string) (model.VoyageLeg, error)
	AddLegUnchecked(context.Context, int64, int64, model.VoyageLeg, string) (model.VoyageLeg, error)
...
func (s *Service) AddLeg(ctx context.Context, actor model.User, planID int64, leg model.VoyageLeg, requestID string) (model.VoyageLeg, error) {
	...
	overlaps, err := s.repository.HasOverlappingLeg(ctx, planID, leg)
	if err != nil {
		return model.VoyageLeg{}, err
	}
	if overlaps {
		return model.VoyageLeg{}, fault.New(fault.Conflict, "leg_window_conflict", "leg overlaps another leg in the plan")
	}
	return s.repository.AddLegUnchecked(ctx, actor.ID, planID, leg, requestID)
}

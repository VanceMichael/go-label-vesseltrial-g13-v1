...
type Repository interface {
	...
	CompleteJobWithToken(context.Context, int64, string) error
	RescheduleJob(context.Context, int64, time.Time) error
	FailJob(context.Context, int64, error, time.Duration) error
	FailJobWithToken(context.Context, int64, string, error, time.Duration) error
...
func (w *Worker) RunOnce(ctx context.Context) error {
	...
	if runErr == nil {
		...
		return w.repository.CompleteJobWithToken(ctx, job.ID, job.ClaimToken)
	}
	shift := time.Second << min(job.Attempts-1, 6)
	if shift > w.maxBackoff {
		shift = w.maxBackoff
	}
	if failErr := w.repository.FailJobWithToken(ctx, job.ID, job.ClaimToken, runErr, shift); failErr != nil {

...
VALUES(?,?,?,?,?,?,?,?,?,?)`, sample.VesselID, sample.LegID, sample.StationID, sample.Sequence, sample.BatteryPct, sample.SpeedKnots, sample.Latitude, sample.Longitude, formatTime(sample.ObservedAt), formatTime(now))
	...
	if hooks := telemetryservice.IngestLifecycleHooksFromContext(ctx); hooks.AfterSampleInsert != nil {
		hooks.AfterSampleInsert()
	}
	if err := ctx.Err(); err != nil {
		return model.TelemetrySample{}, fmt.Errorf("telemetry request canceled before station freshness: %w", err)
	}
	if _, err := s.db.ExecContext(ctx, `UPDATE shore_stations SET last_seen_at=?,status='online' WHERE id=?`, formatTime(now), sample.StationID); err != nil {
		return model.TelemetrySample{}, fmt.Errorf("update station freshness: %w", err)
	}
	return saved, nil
}

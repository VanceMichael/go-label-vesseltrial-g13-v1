...
func (s *Store) AddLeg(ctx context.Context, actorID, planID int64, leg model.VoyageLeg, requestID string) (model.VoyageLeg, error) {
	return s.addLeg(ctx, actorID, planID, leg, requestID, true)
}

func (s *Store) AddLegUnchecked(ctx context.Context, actorID, planID int64, leg model.VoyageLeg, requestID string) (model.VoyageLeg, error) {
	return s.addLeg(ctx, actorID, planID, leg, requestID, false)
}

func (s *Store) addLeg(ctx context.Context, actorID, planID int64, leg model.VoyageLeg, requestID string, checkOverlap bool) (model.VoyageLeg, error) {
	var created model.VoyageLeg
	err := s.InTx(ctx, func(tx *sql.Tx) error {
		if !checkOverlap {
			if _, err := tx.ExecContext(ctx, `UPDATE trial_plans SET updated_at=updated_at WHERE id=?`, planID); err != nil {
				return fmt.Errorf("reserve plan write lock: %w", err)
			}
		}
...
func (s *Store) HasOverlappingLeg(ctx context.Context, planID int64, leg model.VoyageLeg) (bool, error) {
	plan, err := s.Plan(ctx, planID)
	if err != nil {
		return false, err
	}
	if plan.Status != "draft" {
		return false, fault.New(fault.Conflict, "plan_not_draft", "legs can only be added to a draft plan")
	}
	var overlaps int
	err = s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM voyage_legs WHERE plan_id=? AND starts_at<? AND ends_at>?`, planID, formatTime(leg.EndsAt), formatTime(leg.StartsAt)).Scan(&overlaps)
	if err != nil {
		return false, fmt.Errorf("check leg overlap: %w", err)
	}
...
func (s *Store) CompleteLeg(ctx context.Context, actorID, legID, expectedVersion int64, requestID string) (model.VoyageLeg, error) {
	...
	err := s.InTx(ctx, func(tx *sql.Tx) error {
		...
		if err := audit.New().Write(ctx, tx, actorID, "voyage_leg", legID, "complete", "success", requestID, "trial observations accepted"); err != nil {
			return err
		}
		leg, err = loadLeg(ctx, tx, legID)
		return err

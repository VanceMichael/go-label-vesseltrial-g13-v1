...
func (s *Store) HasActiveWindow(ctx context.Context, legID int64) (bool, error) {
	current, err := s.Leg(ctx, legID)
	if err != nil {
		return false, err
	}
	var conflicts int
	err = s.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM voyage_legs WHERE vessel_id=? AND id<>? AND status IN ('released','underway') AND starts_at<? AND ends_at>?`, current.VesselID, legID, formatTime(current.EndsAt), formatTime(current.StartsAt)).Scan(&conflicts)
	if err != nil {
		return false, fmt.Errorf("check release window: %w", err)
	}
	return conflicts != 0, nil
}

func (s *Store) ReleaseLegUnchecked(ctx context.Context, actorID, legID, expectedVersion int64, requestID string) (model.VoyageLeg, error) {
	return s.releaseLeg(ctx, actorID, legID, expectedVersion, requestID, false)
}

func (s *Store) releaseLeg(ctx context.Context, actorID, legID, expectedVersion int64, requestID string, checkWindow bool) (model.VoyageLeg, error) {
	now := s.now()

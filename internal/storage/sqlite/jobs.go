...
func (s *Store) ClaimJob(ctx context.Context, lease time.Duration) (model.WorkerJob, error) {
	...
	err := s.InTx(ctx, func(tx *sql.Tx) error {
		row := tx.QueryRowContext(ctx, `SELECT id,kind,payload,status,attempts,max_attempts,available_at,locked_until,last_error FROM worker_jobs WHERE status IN ('pending','retry') AND available_at<=? AND (locked_until IS NULL OR locked_until<?) ORDER BY available_at,id LIMIT 1`, formatTime(now), formatTime(now))
		item, err := scanJob(row)
		if err != nil {
			return err
		}
		result, err := tx.ExecContext(ctx, `UPDATE worker_jobs SET status='running',attempts=attempts+1,locked_until=?,updated_at=? WHERE id=? AND status IN ('pending','retry')`, formatTime(lockedUntil), formatTime(now), item.ID)
		if err != nil {
			return err
		}
...
func (s *Store) RescheduleJob(ctx context.Context, id int64, availableAt time.Time) error {
	if err := ctx.Err(); err != nil {
		return fmt.Errorf("reschedule job canceled: %w", err)
	}

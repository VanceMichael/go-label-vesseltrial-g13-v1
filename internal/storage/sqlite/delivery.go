...
var deliveryGateAfterQualification func()
...
func (s *Store) EvaluateDelivery(ctx context.Context, actorID, batchID, version int64, release bool, requestID string) (model.DeliveryBatch, error) {
	now := s.now()
	var saved model.DeliveryBatch
	if marked, _ := ctx.Value("vesseltrial.delivery-gate").(bool); marked {
		return s.evaluateDeliverySplit(ctx, actorID, batchID, version, release, requestID, now)
	}
	err := s.InTx(ctx, func(tx *sql.Tx) error {
...
func (s *Store) evaluateDeliverySplit(ctx context.Context, actorID, batchID, version int64, release bool, requestID string, now time.Time) (model.DeliveryBatch, error) {
	var current model.DeliveryBatch
	if err := s.InTx(ctx, func(tx *sql.Tx) error {
		...
		if deliveryGateAfterQualification != nil {
			deliveryGateAfterQualification()
		}
		return nil
	}); err != nil {
	...
	var releasedAt any
	if release {
		target, releasedBy, releasedAt = "released", actorID, formatTime(now)
	}
	if err := s.InTx(ctx, func(tx *sql.Tx) error {
		result, err := tx.ExecContext(ctx, `UPDATE delivery_batches SET status=?,version=version+1,released_by=?,released_at=? WHERE id=? AND version=? AND status IN ('preparing','quality_gate')`, target, releasedBy, releasedAt, batchID, version)
		if err != nil {
	...
	return s.DeliveryBatch(ctx, batchID)
}

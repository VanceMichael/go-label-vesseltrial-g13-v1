...
func (s *Store) TransitionDefect(ctx context.Context, actorID, defectID, version int64, target, note, requestID string) (model.Defect, error) {
	now := s.now()
	if value, _ := ctx.Value("vesseltrial.final-closure").(bool); value {
		return s.transitionFinalClosure(ctx, actorID, defectID, version, target, note, requestID, now)
	}
	var saved model.Defect
	err := s.InTx(ctx, func(tx *sql.Tx) error {
...
func (s *Store) transitionFinalClosure(ctx context.Context, actorID, defectID, version int64, target, note, requestID string, now time.Time) (model.Defect, error) {
	var saved model.Defect
	if err := s.InTx(ctx, func(tx *sql.Tx) error {
		...
		if current.Status != "verified" || target != "closed" {
			return fault.New(fault.Conflict, "invalid_defect_transition", "defect action does not follow containment and verification order")
		}
		result, err := tx.ExecContext(ctx, `UPDATE defects SET status='closed',version=version+1,updated_at=? WHERE id=? AND version=? AND status='verified'`, formatTime(now), defectID, version)
		if err != nil {
			return fmt.Errorf("close defect: %w", err)
		}
		changed, _ := result.RowsAffected()
		if changed != 1 {
			return fault.New(fault.Conflict, "stale_defect", "defect was changed by another operator")
	...
	if err := s.InTx(ctx, func(tx *sql.Tx) error {
		...
		return audit.New().Write(ctx, tx, actorID, "defect", defectID, target, "success", requestID, note)
	}); err != nil {
		return model.Defect{}, err
	}
	saved, err := s.Defect(ctx, defectID)
	return saved, err
}

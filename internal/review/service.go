...
func (s *Service) Decide(ctx context.Context, actor model.User, reviewID, version int64, decision, reason, requestID string) (model.ClassReview, error) {
	...
	current, err := s.repository.Review(ctx, reviewID)
	if err != nil {
		return model.ClassReview{}, err
	}
	if current.Status != "submitted" {
		return model.ClassReview{}, fault.New(fault.Conflict, "invalid_review_transition", "only submitted reviews can be decided")
	}
	if current.Version != version {
		return model.ClassReview{}, fault.New(fault.Conflict, "stale_review", "review was changed by another surveyor")
	}

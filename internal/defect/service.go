...
func (s *Service) Transition(ctx context.Context, actor model.User, id, version int64, target, note, requestID string) (model.Defect, error) {
	...
	// Final closure carries an explicit operation marker so the repository can
	// keep the state-machine transition and its evidence in one lifecycle.
	if target == "closed" {
		ctx = context.WithValue(ctx, "vesseltrial.final-closure", true)
	}
